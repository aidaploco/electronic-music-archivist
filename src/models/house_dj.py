...
class HouseDJ(BaseModel):
    ...
    active_years: Optional[str] = Field(None, description="The period during which the DJ has been active (e.g., '1980s-present' or '1990-2010').")
    notable_tracks: Optional[List[str]] = Field(None, description="A list of influential or well-known tracks by the DJ.")
    associated_labels: Optional[List[str]] = Field(None, description="Record labels the DJ has been closely associated with.")
    influences: Optional[List[str]] = Field(None, description="Artists or genres that influenced the DJ's style.")
    known_for: Optional[str] = Field(None, description="A brief description of what the DJ is most known for or their signature style.")
    biography_summary: Optional[str] = Field(None, description="A concise summary of the DJ's biography and career highlights.")
    genres: Optional[List[str]] = Field(None, description="Main electronic music genres the DJ is associated with (e.g., 'Chicago House', 'Deep House').")
    website: Optional[HttpUrl] = Field(None, description="Official website or prominent online profile URL.")
    social_media: Optional[Dict[str, str]] = Field(None, description="Dictionary of social media platforms and their URLs (e.g., {'instagram': 'https://instagram.com/djname'}).")
    awards: Optional[List[str]] = Field(None, description="Notable awards or recognitions received by the DJ.")
    collaborations: Optional[List[str]] = Field(None, description="Key artists or producers the DJ has collaborated with.")
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra = { # Generates a JSON schema for the model, useful for instructing LLMs
            "example": {
                ...
                "biography_summary": "Frankie Knuckles was an American DJ, record producer, and remixer. He was instrumental in developing and popularizing house music in Chicago during the 1980s. Often referred to as 'The Godfather of House Music'.",
                ...
                "legacy": "Widely regarded as the creator of house music, his influence is profound and continues to shape electronic music worldwide."
            }
        }
    )
